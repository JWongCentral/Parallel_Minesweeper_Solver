...
void printMoves(vector<moves> vec);
...
vector<moves> ruleThree() {
    ...
    cout << "unknowns" << unknowns.size() <<endl;
    cout << "Possibilities" << check.size() <<endl;
...
void printMoves(vector<moves> vec){
    for(int i = 0; i < vec.size(); i++){
        ...
        if(temp.action == flag) cout << "Action: FLAG"<< endl << endl;\
        else if(temp.action == open) cout << "Action: OPEN"<< endl << endl;
    }
}
...
*/
//There is an html attached to this that you can use to get a custom board state
//Front page is the hidden
//f12->inspect->console
//will print the board answer for testing
